function [imgout, idx2, aligned] = ...
        smooth_by_center_alignment_norm(imgin, ...
                                        centers, ...
                                        crop_radius, ...
                                        scaling, ...
                                        baseline, ...
                                        magnitude, ...
                                        varargin)
%% SMOOTH_BY_CENTER_ALIGNMENT_NORM obtains smoothed images 
% by aligning normalized sub-images around given centers.
...
% baseline: background intensity
% magnitude: mag of the function (after removing the backgound)
...
for jj = 1:L
    ...
    cropped = imgin((ty-r):(ty+r), ...
                    (tx-r):(tx+r), ...
                    jj);
    cropped = (cropped - baseline(jj))/magnitude(jj);
    % rescaling
