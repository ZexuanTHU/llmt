...
% imgroi.a2: similar to a, 8*np*num_frames; filamenttip model
...
% image_viewer(test_img,imgroi2);
    ...
    function delete_arrow()
        if active_ind>0 && active_ind<=size(imgroi.a,2)
            ...
            imgroi.a2(:,active_ind,cur_frame) = nan;
    ...
    function op_on_point()
        if active_ind>0 && active_ind<=size(imgroi.p,2)
            ...
            [p,param] = op_localize_point(imgs(:,:,cur_frame),p0,showinfo_flag);
            if sum(isnan(p))==0
    ...
    % NOTE: the value at the first frame must be initialized!
        if active_ind>0 && active_ind<=size(imgroi.p,2)
            disp('Computing: Localizing all points'); % start
            tic;
            init_frame = 1;
            p0 = imgroi.p(:,active_ind,init_frame);
            [p,param0] = op_localize_point(...
                imgs(:,:,init_frame), ...
                p0, ...
                showinfo_flag);
            if sum(isnan(p))==0
                params = op_localize_all_points(...
                    imgs,...
                    param0,...
                    showinfo_flag);
                imgroi.p(:,active_ind,:) = params(1:2,:,:);
                update_point();
            end
            disp('done'); % end
            toc;
    ...
    function op_on_arrow()
        if active_ind>0 && active_ind<=size(imgroi.a,2)
            ...
            if sum(isnan(p))==0
                ...
                imgroi.a2(:,active_ind,cur_frame) = param;
                update_arrow();
            end
        end
    end
    ...
    % disp('hello all arrows');
        % NOTE: the value at the first frame must be initialized!
        if active_ind>0 && active_ind<=size(imgroi.a,2)
            disp('Computing: Localizing all arrows'); % start
            tic;
            op_on_arrow();
            cur_frame = getappdata(h_fig,'cur_frame');
            params = squeeze(imgroi.a2(:,active_ind,:));
            params = op_localize_all_endpoints(imgs,...
                                               params,...
                                               cur_frame,...
                                               showinfo_flag);
            p1 = params(1:2,:);
            p2 = p1 + ABL*[cos(params(4,:)+pi);...
                           sin(params(4,:)+pi)];
            p12 = reshape([p1;p2],[4,1,num_frames]);
            imgroi.a(:,active_ind,:) = p12;
            imgroi.a2(:,active_ind,:) = reshape(params,[8,1, ...
                                num_frames]);
            disp('done'); % end
            toc;
        end
